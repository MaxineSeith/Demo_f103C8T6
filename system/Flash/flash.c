 ...
 * @retval      读取到的数据 (16位)
 */
uint16_t Flash_ReadData_halfword(uint32_t faddr)
{
 ...
 * @retval      无
 */
void Flash_ReadData(uint32_t raddr, uint16_t *pbuf, uint16_t length)
{
    uint16_t i;

    for (i = 0; i < length; i++)
    {
        pbuf[i] = Flash_ReadData_halfword(raddr);    /* 读取2个字节 */
        raddr += 2; /* 偏移2个字节 */
    }
}
...
void Flash_WriteData(uint32_t waddr, uint16_t *pbuf, uint16_t length)
{
    ...
    while (1)
    {
        Flash_ReadData(secpos * STM32_SECTOR_SIZE + STM32_FLASH_BASE, g_flashbuf, STM32_SECTOR_SIZE / 2); /* 读出整个扇区的内容 */
...
{
    Flash_WriteData(waddr, &wdata, 1); /* 写入一个半字 */
}
