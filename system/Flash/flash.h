...
#define DATA_EEPROM_START_ADDR  0x08080000  //EEPROM
/* FLASH起始地址 */
...
/* 接口函数(外部可调用) */
uint16_t Flash_ReadData_halfword(uint32_t faddr);                        /* FLASH读半字 */
void Flash_ReadData(uint32_t raddr, uint16_t *pbuf, uint16_t length);    /* 从指定地址开始读出指定长度的数据 */
void Flash_WriteData(uint32_t waddr, uint16_t *pbuf, uint16_t length);   /* 在FLASH 指定位置, 写入指定长度的数据(自动擦除) */
